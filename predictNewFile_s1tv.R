...
#rbind all results
totalResults_S1TV <- train[,c(1,8,37:40)]
totalResults_S1TV <- rbind(totalResults_S1TV, test[,c(1,10,37:40)])

#write date & preds in a csv
write.csv2(totalResults_S1TV,"totalPredictions_S1TV.csv")


#### plot ####
p <- plot_ly(totalResults_S1TV,x=totalResults_S1TV$Date, y=totalResults_S1TV$n, name="Actuals") %>% 
  add_trace(x=totalResults_S1TV$Date, y=totalResults_S1TV$preds, name="RandomForest") %>%
  add_trace(x=totalResults_S1TV$Date, y=totalResults_S1TV$p_n, name="Linear Regression") %>% layout(title="S1TV STREAMSTARTS PREDICTION",xaxis=list(title='Date'),yaxis=list(title='StreamStarts'))
p
