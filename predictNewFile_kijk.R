...
#rbind all results
totalResults_KIJK <- train[,c(1,8,37:40)]
totalResults_KIJK <- rbind(totalResults_KIJK, test[,c(1,10,37:40)])

#write date & preds in a csv
write.csv2(totalResults_KIJK,"totalPredictions_KIJK.csv")


#### plot ####
p <- plot_ly(totalResults_KIJK,x=totalResults_KIJK$Date, y=totalResults_KIJK$n, name="Actuals") %>% 
  add_trace(x=totalResults_KIJK$Date, y=totalResults_KIJK$preds, name="RandomForest") %>%
  add_trace(x=totalResults_KIJK$Date, y=totalResults_KIJK$p_n, name="Linear Regression")  %>% layout(title="KIJK STREAMSTARTS PREDICTION",xaxis=list(title='Date'),yaxis=list(title='StreamStarts'))
p
