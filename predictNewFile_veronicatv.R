...
#rbind all results
totalResults_VERONICATV <- train[,c(1,8,37:40)]
totalResults_VERONICATV <- rbind(totalResults_VERONICATV, test[,c(1,10,37:40)])

#write date & preds in a csv
write.csv2(totalResults_VERONICATV,"totalPredictions_VERONICATV.csv")


#### plot ####
p <- plot_ly(totalResults_VERONICATV,x=totalResults_VERONICATV$Date, y=totalResults_VERONICATV$n, name="Actuals") %>% 
  add_trace(x=totalResults_VERONICATV$Date, y=totalResults_VERONICATV$preds, name="RandomForest") %>%
  add_trace(x=totalResults_VERONICATV$Date, y=totalResults_VERONICATV$p_n, name="Linear Regression")  %>% layout(title="VERONICATV STREAMSTARTS PREDICTION",xaxis=list(title='Date'),yaxis=list(title='StreamStarts'))
p
