...
#rbind all results
totalResults_NET5 <- train[,c(1,8,37:40)]
totalResults_NET5 <- rbind(totalResults_NET5, test[,c(1,10,37:40)])

#write date & preds in a csv
write.csv2(totalResults_NET5,"totalPredictions_NET5.csv")


#### plot ####
p <- plot_ly(totalResults_NET5,x=totalResults_NET5$Date, y=totalResults_NET5$n, name="Actuals") %>% 
  add_trace(x=totalResults_NET5$Date, y=totalResults_NET5$preds, name="RandomForest") %>%
  add_trace(x=totalResults_NET5$Date, y=totalResults_NET5$p_n, name="Linear Regression") %>% layout(title="NET5 STREAMSTARTS PREDICTION",xaxis=list(title='Date'),yaxis=list(title='StreamStarts'))
p
