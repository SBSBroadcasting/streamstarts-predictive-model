...
#rbind all results
totalResults_SBS9 <- train[,c(1,8,37:40)]
totalResults_SBS9 <- rbind(totalResults_SBS9, test[,c(1,10,37:40)])

#write date & preds in a csv
write.csv2(totalResults_SBS9,"totalPredictions_SBS9.csv")


#### plot ####
p <- plot_ly(totalResults_SBS9,x=totalResults_SBS9$Date, y=totalResults_SBS9$n, name="Actuals") %>% 
  add_trace(x=totalResults_SBS9$Date, y=totalResults_SBS9$preds, name="RandomForest") %>%
  add_trace(x=totalResults_SBS9$Date, y=totalResults_SBS9$p_n, name="Linear Regression") %>% layout(title="SBS9 STREAMSTARTS PREDICTION",xaxis=list(title='Date'),yaxis=list(title='StreamStarts'))
p
