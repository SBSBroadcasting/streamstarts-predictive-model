...
library('lubridate')
library(dplyr)
# weather <- read.delim('weather15-16.txt',header = TRUE,sep = ',')
# weather_data <- weather[,-c(1)]
# weather_data$YYYYMMDD <- ymd(weather_data$YYYYMMDD)
# weather_data$TG <- weather_data$TG/10
...
for(i in 1:12){
  ...
  # wresults$Temp <- gsub("Â°","",wresults$Temp)
  # wresults$Rain <- gsub(" mm","",wresults$Rain)
  print(wresults[[i]])
...
library(dplyr)
tw <- weather_data
#tw <- unique(rbind(tw, weather))
tw$Date <- ymd(tw$Date)
tw$Temp <- gsub("°","",tw$Temp)
tw$Temp <- as.numeric(tw$Temp)
tw$Rain <- as.numeric(tw$Rain)
# tw <- mutate(tw,Temp=ifelse(is.na(Temp)==TRUE,mean(Temp),Temp))
# tw <- mutate(tw,Rain=ifelse(is.na(Rain)==TRUE,mean(Rain),Rain))

# tw <- group_by(tw, Date)
# tw <- summarize(tw, Temp = mean(Temp),Rain=mean(Rain))
