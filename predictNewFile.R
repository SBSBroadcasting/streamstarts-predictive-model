...
#rbind all results
totalResults <- train[,c(1,8,37:40)]
totalResults <- rbind(totalResults, test[,c(1,10,37:40)])

#write date & preds in a csv
write.csv2(totalResults,"totalPredictions.csv")


#### plot ####
p <- plot_ly(totalResults,x=totalResults$Date, y=totalResults$n, name="Actuals") %>% 
  add_trace(x=totalResults$Date, y=totalResults$preds, name="RandomForest") %>%
  add_trace(x=totalResults$Date, y=totalResults$p_n, name="Linear Regression")
p
